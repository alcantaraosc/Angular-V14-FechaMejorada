...
export class ListafuncionesComponent implements OnInit, AfterViewInit {
  ...
  onListarFunciones() 
  {
    ...
    this.accesosecurityfunciones.getListarFunciones().subscribe((post: any) => {
  ...
  onEditarDatosFunciones(funcionesID: number) {
    if (confirm('¿ Estas seguro de editar el registro de la funcion ?')) {
      ...
      this.accesosecurityfunciones.getDatoFuncionesPorId(funcionesID).subscribe((response: responseModel) => {
  ...
  getDatosPorFiltro(e, nombrefunciones: string) {
    ...
    if (e.keyCode === 13) {
      if (nombrefunciones.length > 0) {
        //habilitar la ruedita cargando
        this.emitirDatosService.habilitarCargando();
        this.accesosecurityfunciones.getDatosFuncionesPorNombre(nombrefunciones).subscribe((response: responseModel) => {
              //habilitar 
              this.emitirDatosService.desactivarCargando();
